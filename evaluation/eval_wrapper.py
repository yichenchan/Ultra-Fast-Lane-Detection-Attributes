...
def generate_lines(out, shape, names, output_path, num_x_grid, localization_type='abs', flip_updown=False):

    col_sample = np.linspace(0, shape[1] - 1, num_x_grid)
    ...
    for j in range(out.shape[0]):
        ...
        if localization_type == 'abs':
            ...
            out_j[out_j == num_x_grid] = -1
            out_j = out_j + 1
        elif localization_type == 'rel':
            prob = scipy.special.softmax(out_j[:-1, :, :], axis=0)
            idx = np.arange(num_x_grid) + 1
            idx = idx.reshape(-1, 1, 1)
            loc = np.sum(prob * idx, axis=0)
            out_j = np.argmax(out_j, axis=0)
            loc[out_j == num_x_grid] = 0
            out_j = loc
        else:
            ...
            for i in range(out_j.shape[1]):
                if np.sum(out_j[:, i] != 0) > 2:
                    for k in range(out_j.shape[0]):
                        if out_j[k, i] > 0:
                            fp.write('%d %d ' % (int(out_j[k, i] * col_sample_w * 2560 / 800) - 1, int(1440 - k * 20) - 1))
                    fp.write('\n')


def run_test(net, dataloader, exp_name, work_dir, num_x_grid, use_aux, distributed, batch_size=8):
    ...
    for i, data in enumerate(dist_tqdm(dataloader)):
        imgs, names = data
        imgs = imgs.cuda()
        with torch.no_grad():
            out = net(imgs)
        ...
        generate_lines(out[0], imgs[0,0].shape, names, output_path, num_x_grid, localization_type = 'rel', flip_updown = True)
        

def eval_lane(net, dataloader, work_dir, num_x_grid, use_aux, distributed):
    net.eval()
    run_test(net, dataloader, 'lane_eval_tmp', work_dir, num_x_grid, use_aux, distributed)
    synchronize()   # wait for all results

    # if is_main_process():
    #     res = call_lane_eval(data_root, 'lane_eval_tmp', work_dir)
    #     TP,FP,FN = 0,0,0
...
def call_lane_eval(data_dir, exp_name, output_path):
    if data_dir[-1] != '/':
        data_dir = data_dir + '/'
    ...
    eval_cmd = './evaluation/lane/evaluate'
